...
// let value = `|   91 distinct values for attribute #0 (age) continuous
...
//   .map((it) => {
//     const res =
//       /(\d+)\s+distinct\s+values\s+for\s+attribute\s+#\d+\s+\(([\w\s]+)\)\s(\w+)/.exec(
//         it
//       );
//     // console.log(res);
//     if (res) {
//       let [, numberOfValue, name, type] = res;
//       name = name.replaceAll(" ", "_");
//       return { numberOfValue, name, type };
//     }
//     return undefined;
//     // console.log(numberOfValue, name, type);
//   })
//   .filter((it) => it !== undefined);

// console.log(split.map(it => it.name));

let a = readFileSync(
  `D:\\Document\\Nam4\\Nam4_HK2\\ChuyenDe\\census-income.csv`,
  {
    encoding: "utf8",
  }
);

let data = a.split("\n").map(it => it.split(",").map(it => it.trim().replace('-', '_').replaceAll(' ', '_')));

writeFileSync(
  `D:\\Document\\Nam4\\Nam4_HK2\\ChuyenDe\\census-income1.csv`,
  data.map(it => it.join(",")).join("\n"),
  {
    encoding: "utf8",
  }
);
...
// let re = new Array(data[0].split(",").length);
...
//   it.split(",").forEach((it1, index1) => {
//     re[index1].push(it1.trim());
//   });
// });
a = `age: continuous.
...
split = a.map(it => {
  ...
  if (res) {
    ...
    let [, name, type] = res;
    name = name.replaceAll(" ", "_");
    type = type.replace(/,\s+/g, ',').replaceAll(" ", "_").replaceAll('-', '_');
...
let res = split
  .map((it, i) => {
    if (it.type == "continuous") {
      return `@attribute ${it.name} numeric`;
    } else {
      // let l = [...new Set(re[i])];
      // // console.log(l.length, it.numberOfValue, it.name);
      // if (l.length != it.numberOfValue) {
      //   console.log(it.name);
      // }
      return `@attribute ${it.name} {${it.type}}`;
    }
  })
  .join("\n");
